    ...
    <div class="success-card bg-white rounded-3xl border border-gray-100 shadow-card overflow-hidden">
        ...
        <div class="px-6 sm:px-8 py-8 space-y-6">
            ...
            <div class="rounded-2xl border border-gray-100 divide-y divide-gray-50 overflow-hidden">
                @php
                $lastPaid = $enrollment->payments->where('status', 'paid')->sortByDesc('paid_at')->first();
                $lastCheckoutPesos = $lastPaid ? ((int) $lastPaid->amount / 100) : (int) $enrollment->total_amount;
                ...
                $details = [
                    ...
                    ['label' => 'Last successful checkout',   'value' => '₱' . number_format($lastCheckoutPesos), 'icon' => 'M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z'],
                ...
                if ($enrollment->payment_type === 'downpayment') {
                    ...
                    array_splice($details, 4, 0, [
                        ...
                        ['label' => 'Remaining tuition', 'value' => '₱' . number_format($bal), 'hint' => $bal > 0 ? 'Pay remaining tuition before the early-bird rate ends to use the discounted total, if applicable.' : null, 'icon' => 'M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 5h.01M12 12h3m-3 4h.01M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z'],
                    ]);
                }
                if(!empty($schedule)) {
                ...
                <a href="{{ $payBalanceUrl }}"
                   ...
                   class="success-cta order-first flex flex-1 flex-col items-center justify-center gap-2 px-4 py-4 min-h-[5.25rem] bg-accent-500 text-brand-950 font-extrabold rounded-xl shadow-sm hover:bg-accent-400 transition-all duration-200 text-center">
                    ...
                    <span class="text-sm leading-snug">
                        <span class="block">Pay remaining balance</span>
